...
ascending** — `FR-1..FR-n` and `TC-1..TC-n` in table order, with no gaps and no
inserts in the middle of a table. `spec-md lint` enforces this. The default
edit adds `n + 1` at the end. If you reorder rows, renumber them `1..n` in the
new order. Then update every related `[TC-N]` test tag in the same change.
...
4. **When you update, read the current spec first.** Compare its claims against
   ...
   no `FR-N`, requirements whose behavior changed, `TC-N` rows that no longer
...
`TC-N`, and the review briefings in Step 5. The full dictionary and all 65
...
A table of `TC-N` rows. Each row cites the `FR-N` it proves. Usually
**several** test cases prove one FR (for example, pricing needs a case for
size, one for quantity, and one for rounding). Each row is a deterministic,
...
| TC-2 | FR-2 | The customer orders a large pizza | The unit price is the base price × 1.6, rounded to the cent |
| TC-3 | FR-4 | The request has no `customerId` | The API returns status 400 |
| TC-4 | FR-4 | The `items` list is empty | The API returns status 400 |
```

Cover the happy path first, then the edge cases, then the error conditions.
Write the related cases under the FR they belong to, **in the order you want
them to keep** — the ids become `TC-1..TC-n` in that table order. Be concrete:
`[owner, downlineA, unrelated]` is better than "a mixed array".
...
- **Keep ids contiguous and ascending.** Every FR/TC table must be exactly
  `FR-1..FR-n` / `TC-1..TC-n` in row order, with no gaps and no rows out of
  order. If not, `spec-md lint` fails.
- **Default: append.** A new requirement or case takes `n + 1` and goes at the
  **end** of the table. Count the rows in the table, and confirm that the count
  agrees with the highest `N`, before you allocate an id. Never invent an id in
  the middle of the range (`TC-84` when the table ends at `TC-4`). Never reuse
  an id that is in use, and never copy an id from a different spec.
- **Clean up after each insert, removal, or edit.** If rows are out of order,
  have gaps, or are grouped incorrectly:
  1. Reorder them for readability — FR by domain or lifecycle, TC by the
     `FR-N` they prove (happy path, then edge, then error, in each group).
  2. **Renumber** the table to `1..n` in that new order.
  3. Update every `[TC-N]` test tag, and each review briefing that cites an id,
     in the same change.
  Do not leave `TC-1, TC-2, TC-84, TC-3`. Do not renumber without an update to
  the tags.
- **Edit a row in place** when the behavior changed but the requirement is the
  same. Update the text of the row, not its number.
- **Mark removed behavior** instead of a silent deletion: add `[REMOVED]` to
  the row and delete its `[TC-N]` tests, so that reviewers see the change. A
  `[REMOVED]` row keeps its index, and the sequence stays contiguous. When
  nothing refers to it, delete the row and compact the table: renumber the
  remaining rows to `1..n` and update the tags.
...
- **Lint at the end.** Run `spec-md lint` (or `check`) on the file. Correct each
  sequence error, duplicate id, and dangling reference before you call the
  update done.

## Step 3: Link the tests

A test traces back to the spec through a bracketed `[TC-N]` prefix in the test
name. The tag is the join key between the spec and the suite. See
[TESTING.md](https://github.com/rosenjcb/spec.md/blob/main/TESTING.md) for the
full convention.

```ts
it("[TC-3] Given a request with no customerId, when the store creates the order, then it throws a ValidationError", () => { ... });
```

Coverage is greppable: each `TC-N` in the spec must have one `[TC-N]` test or
more. A test with no matching row is a signal to add the `TC-N` row. If the
...
- Flag each `FR-N` with no `TC-N`, and each `TC-N` with no `[TC-N]` test.
- Flag duplicate ids, gaps in the numbers, and FR/TC rows that are out of
  order. To correct them: reorder the rows, renumber them `1..n`, and update
  the `[TC-N]` tags before you finish.
...
3. Write the body: one instruction paragraph that states the goal, a roles
   ...
   `FR-N`/`TC-N` rows that it summarizes. Never write a restatement by hand,
...
`FR-N`/`TC-N` id. Git history keeps the old round, so do not append rounds to
the file.
...
- **Short first, deep later.** More prose makes a reader skim, and a spec that
  ...
  `FR-N`, and `TC-N`. Link out for depth — [TESTING.md](./TESTING.md),
...
- **Table order.** After each insert, removal, or edit, the tables must stay
  `FR-1..FR-n` / `TC-1..TC-n` in ascending order with no gaps. Reorder for
  readability, then renumber and update the `[TC-N]` tags. Lint is the gate.
